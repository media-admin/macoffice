...
class TINVWL_Trigger_Wishlist_Reminder extends AutomateWoo\Triggers\AbstractBatchedDailyTrigger
{
	...
	{
		$this->title = sprintf(__('Wishlist Reminder (TI WooCommerce Wishlist)', 'ti-woocommerce-wishlist-premium'));
	...
	function load_fields()
	{
		...
		$period->set_title(__('Reminder interval (days)', 'ti-woocommerce-wishlist-premium'));
		$period->set_description(__('E.g. Reminder any customers with items in a Wishlist every 30 days.', 'ti-woocommerce-wishlist-premium'));
		$period->set_required();

		$once_only = new AutomateWoo\Fields\Checkbox();
		$once_only->set_name('once_only');
		$once_only->set_title(__('Once per customer', 'ti-woocommerce-wishlist-premium'));
		$once_only->set_description(__('If checked the trigger will fire only once for each customer for each wishlist they create. Most customers only use the one wishlist so use with caution. Setting a high Reminder interval may be a better plan.', 'ti-woocommerce-wishlist-premium'));
