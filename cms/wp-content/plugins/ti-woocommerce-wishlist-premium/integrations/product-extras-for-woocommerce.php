						...
						foreach ( $group['items'] as $item ) {
							...
							if ( isset( $item['field_type'] ) ) {
								...
								if ( ! empty( $item['field_flatrate'] ) ) {
									$price = '<span class="pewc-flat-rate-cart-label">(' . __( 'Flat rate cost', 'ti-woocommerce-wishlist-premium' ) . ')</span>';
								}
...
if ( ! function_exists( 'tinvwl_item_price_pewc' ) ) {
	 ...
	 * @return string
	 */
	function tinvwl_item_price_pewc( $price, $wl_product, $product, $raw ) {
	...
	add_filter( 'tinvwl_wishlist_item_price', 'tinvwl_item_price_pewc', 10, 4 );
} // End if().
