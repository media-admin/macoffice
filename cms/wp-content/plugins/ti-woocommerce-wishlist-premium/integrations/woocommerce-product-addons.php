...
if ( ! function_exists( 'tinvwl_item_price_woocommerce_product_addons' ) ) {
	 ...
	 * @return string
	 */
	function tinvwl_item_price_woocommerce_product_addons( $price, $wl_product, $product, $raw ) {
		...
		if ( class_exists( 'WC_Product_Addons' ) ) {
			...
			if ( $product_addons ) {
				...
				if ( $raw ) {
					return (float) $product->get_price() + (float) $price;
				}
	...
	add_filter( 'tinvwl_wishlist_item_price', 'tinvwl_item_price_woocommerce_product_addons', 10, 4 );
} // End if().
