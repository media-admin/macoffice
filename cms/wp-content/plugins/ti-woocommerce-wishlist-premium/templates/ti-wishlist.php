						...
						<?php if ( ( isset( $wishlist_table_row['move'] ) && $wishlist_table_row['move'] ) || ( ! isset( $wishlist_table_row['add_to_cart'] ) || $wishlist_table_row['add_to_cart'] ) ) { ?>
							<td class="product-action">
								...
								if ( apply_filters( 'tinvwl_wishlist_item_action_move', $wishlist_table_row['move'], $wl_product, $product ) ) {
									echo apply_filters( 'tinvwl_wishlist_item_move', sprintf( '<button class="button tinvwl-button tinvwl_move_product_button" type="button" name="move"><i class="ftinvwl ftinvwl-arrow-right"></i><span class="tinvwl-txt">%s</span></button>', esc_html( __( 'Move', 'ti-woocommerce-wishlist-premium' ) ) ), $wl_product, $product, $wishlist ); // WPCS: xss ok.
								} ?>
							</td>
						<?php } ?>
					</tr>
					<?php do_action( 'tinvwl_wishlist_row_after', $wl_product, $product );
				} // End if().
			} // End foreach().
