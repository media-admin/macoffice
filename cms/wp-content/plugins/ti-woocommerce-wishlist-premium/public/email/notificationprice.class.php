 ...
 * Wishlist Notification a change price product create email
 */
class TInvWL_Public_Email_NotificationPrice extends WC_Email {
	...
	function __construct( $plugin_name, $version ) {
		...
		$this->settings_class = new TInvWL_Public_Email_Data_Notificationprice;
	...
	function load_data() {
		...
		add_action( 'tinvwl_send_notification_price', array( $this, 'trigger' ), 10, 4 );
	...
	function trigger( $product, $user_id, $wishlists = array(), $coupon = null ) {
		...
		$this->content       = apply_filters( 'tinvwl_prepare_price_content', $this->get_option( 'content' ) );
		$this->content_plain = apply_filters( 'tinvwl_prepare_price_content_plain', $this->get_option( 'content_plain' ) );
		...
		$notifications_allowed = ( isset( $notification_settings['price'] ) && ! empty( $notification_settings['price'] ) && 'unsubscribed' === $notification_settings['price'] ) || ( ! isset( $notification_settings['price'] ) && ! $global_notifications ) ? false : true;
		...
		if ( is_wp_error( $result ) ) {
			do_action( 'tinvwl_send_notification_price_error', $this->get_recipient(), $this->user, $this->product, $result->get_error_message() );
		} else {
			do_action( 'tinvwl_send_notification_price_successfully', $this->get_recipient(), $this->user, $this->product );
		}
	}
