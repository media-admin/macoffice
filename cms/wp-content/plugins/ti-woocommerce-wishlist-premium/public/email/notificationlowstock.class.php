<?php
/**
 * Wishlist Notification a low stock product create email
 *
 * @since             2.0.14
...
/**
 * Wishlist Notification a change stock product create email
 */
class TInvWL_Public_Email_NotificationLowStock extends WC_Email {
	/**
	...
	function __construct( $plugin_name, $version ) {
		...
		$this->settings_class = new TInvWL_Public_Email_Data_Notificationlowstock();
	...
	function load_data() {
		...
		add_action( 'tinvwl_send_notification_low_stock', array( $this, 'trigger' ), 10, 4 );
	...
	function trigger( $product, $user_id, $wishlists = array(), $coupon = null ) {
		...
		$this->content       = apply_filters( 'tinvwl_prepare_low_stock_content', $this->get_option( 'content' ) );
		$this->content_plain = apply_filters( 'tinvwl_prepare_low_stock_content_plain', $this->get_option( 'content_plain' ) );
		...
		$notifications_allowed = ( isset( $notification_settings['low_stock'] ) && ! empty( $notification_settings['low_stock'] ) && 'unsubscribed' === $notification_settings['low_stock'] ) || ( ! isset( $notification_settings['low_stock'] ) && ! $global_notifications ) ? false : true;
		...
		if ( is_wp_error( $result ) ) {
			do_action( 'tinvwl_send_notification_low_stock_error', $this->get_recipient(), $this->user, $this->product, $result->get_error_message() );
		} else {
			do_action( 'tinvwl_send_notification_low_stock_successfully', $this->get_recipient(), $this->user, $this->product );
		}
	}
	...
	public function get_content_html() {
		ob_start();
		tinv_wishlist_template( $this->template_html, apply_filters( 'tinvwl_notification_low_stock_template_html', array(
	...
	public function get_content_plain() {
		ob_start();
		tinv_wishlist_template( $this->template_plain, apply_filters( 'tinvwl_notification_low_stock_data_template_plain', array(
			'email_heading' => $this->get_heading(),
			'blogname'      => $this->get_blogname(),
