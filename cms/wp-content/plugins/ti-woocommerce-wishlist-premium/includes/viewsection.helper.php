...
class TInvWL_ViewSection extends TInvWL_View {
	...
	function section_data( $content ) {
		...
		if ( array_key_exists( 'tiwl-required', $content ) ) {
			$content['tiwl-required'] = esc_attr( json_encode( $content['tiwl-required'] ) );
		}
		$data['extra']       = $content;
	...
	function validation_type( $type ) {
		...
		$types = array(
			...
			'color'         => array(
				...
				'options' => array(
					'regexp'  => '/(\#[0-9a-f]{6}|rgba\((\d{1,3})\,\s*(\d{1,3})\,\s*(\d{1,3})\,\s*([0-9\.]+)\s*\)|rgb\((\d{1,3})\,\s*(\d{1,3})\,\s*(\d{1,3})\s*\))/i',
					'default' => '#000000',
				),
			),
